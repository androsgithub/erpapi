...
/**
 * Holder para gerenciar dinamicamente a instância de ContatoService
 ...
 * - Dependency Inversion: Trabalha com IContatoService (abstração)
...
public class ContatoServiceHolder {

    private final AtomicReference<IContatoService> serviceReference;
     ...
     */
    public IContatoService getService() {
        lock.readLock().lock();
        try {
            IContatoService service = serviceReference.get();
            if (service == null) {
                throw new IllegalStateException("ContatoService não foi inicializado");
            }
            return service;
        } finally {
     ...
     */
    public void updateService(IContatoService newService) {
        lock.writeLock().lock();
        try {
            IContatoService oldService = serviceReference.getAndSet(newService);
            log.info("[CONTATO SERVICE HOLDER] Serviço atualizado. " +
     ...
     */
    public IContatoService getPeekService() {
