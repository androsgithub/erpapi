...
INSERT INTO TB_CUSTOM_DATA (value, custom_field_id, entity_id, custom_data_id)
SELECT 'PREMIUM',
       cfd.id,
       c.id,
       NULL
FROM TB_custom_field_definition cfd,
     tb_business_partner c
WHERE cfd.field_key = 'categoria_cliente'
...
FROM TB_custom_field_definition cfd,
     tb_business_partner c
WHERE cfd.field_key = 'cliente_ativo_marketing'
  AND c.nome = 'DISTRIBUIDORA BRASIL'
LIMIT 1;
...
INSERT INTO TB_CUSTOM_DATA (value, custom_field_id, entity_id, custom_data_id)
SELECT 'Cliente reclamou sobre prazo de entrega em Janeiro/2025',
       cfd.id,
       c.id,
       NULL
FROM TB_custom_field_definition cfd,
     tb_business_partner c
WHERE cfd.field_key = 'observacoes_internas'
  AND c.nome = 'LOJA CENTRAL'
LIMIT 1;
...
FROM TB_custom_field_definition cfd,
     tb_product p
WHERE cfd.field_key = 'produto_promocional'
...
FROM TB_custom_field_definition cfd,
     tb_product p
WHERE cfd.field_key = 'margem_lucro_meta'
...
FROM TB_custom_field_definition cfd,
     tb_product p
WHERE cfd.field_key = 'fornecedor_preferido'
  AND p.codigo = 'PROD002'
LIMIT 1;
...
FROM TB_custom_field_definition cfd,
     tb_product p
WHERE cfd.field_key = 'data_validade_relevante'
...
INSERT INTO TB_CUSTOM_DATA (value, custom_field_id, entity_id, custom_data_id)
SELECT 'ISO9001|FCC|CE',
       cfd.id,
       p.id,
       NULL
FROM TB_custom_field_definition cfd,
     tb_product p
WHERE cfd.field_key = 'certificacoes_produto'
  AND p.codigo = 'PROD005'
LIMIT 1;
...
FROM TB_custom_field_definition cfd,
     tb_business_partner c
WHERE cfd.field_key = 'volume_anual_estimado'
  AND c.nome = 'DISTRIBUIDORA BRASIL'
LIMIT 1;
